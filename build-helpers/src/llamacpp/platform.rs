...
fn windows_config(_target: &str) -> PlatformConfig {
    let mut cmake_flags = HashMap::new();
    let env_vars = HashMap::new();

    // Use Ninja generator for better performance (plain Ninja is the safest on Windows)
    cmake_flags.insert("CMAKE_GENERATOR".to_string(), "Ninja".to_string());

    // Do NOT force an LLVM toolchain on Windows by default. MSVC/ClangCL + VS Build Tools/Windows SDK
    // provide the correct libraries. (If you ever need the custom toolchain, we can guard it by env.)
    // if std::env::var("USE_LLVM_TOOLCHAIN").ok().as_deref() == Some("1") {
    //     let toolchain = if target.contains("aarch64") || target.contains("arm64") {
    //         "cmake/arm64-windows-llvm.cmake"
    //     } else {
    //         "cmake/x64-windows-llvm.cmake"
    //     };
    //     cmake_flags.insert("CMAKE_TOOLCHAIN_FILE".to_string(), toolchain.to_string());
    // }
    ...
    // Do NOT add GNU-style linker flags on Windows (they confuse lld-link/MSVC)
    // (Intentionally not setting CMAKE_EXE_LINKER_FLAGS here.)
