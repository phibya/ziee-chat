...
fn get_comprehensive_backend_config(
    ...
    // CPU-only by default; set ZIEE_USE_GPU=1|true|on to enable GPU backends
    if target.contains("darwin") || target.contains("apple") {
        return get_comprehensive_macos_config(target);
    }

    // If GPU not explicitly requested, build CPU-only
    if !crate::llamacpp::backend::wants_gpu() {
        return crate::llamacpp::backend::get_backend_config(
            crate::llamacpp::backend::BackendType::CPU,
            target,
        );
    }

    // GPU requested on Windows/Linux: go multi-backend
...
fn get_comprehensive_macos_config(
    ...
    let use_gpu = crate::llamacpp::backend::wants_gpu();

    if target.contains("aarch64") || target.contains("arm64") {
        // Apple Silicon: Accelerate always; Metal only when GPU is requested
        ...
        // Intel Mac: Accelerate + SIMD optimizations (no Metal)
        cmake_flags.insert("GGML_ACCELERATE".to_string(), "ON".to_string());
    ...
    Ok(BackendConfig {
        ...
        backend_type: if (target.contains("aarch64") || target.contains("arm64")) && use_gpu {
            BackendType::Metal
        } else {
            BackendType::BLAS
        },
...
fn build_with_config(config: &BuildConfig) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    // Safety: if GPU wasn't explicitly requested, force all GPU backends OFF
    if !crate::llamacpp::backend::wants_gpu() {
        cmake_cmd.arg("-DGGML_CUDA=OFF");
        cmake_cmd.arg("-DGGML_VULKAN=OFF");
        cmake_cmd.arg("-DGGML_OPENCL=OFF");
        cmake_cmd.arg("-DGGML_METAL=OFF");
    }
    ...
    cmake_cmd.arg("-DGGML_BACKEND_DL=OFF");
    cmake_cmd.arg("-DGGML_CPU_ALL_VARIANTS=OFF");
