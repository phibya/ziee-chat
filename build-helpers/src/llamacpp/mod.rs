...
fn build_with_config(config: &BuildConfig) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    println!("Building llama.cpp with backends [{}] for {}", backend_names.join(", "), config.target);
    ...
    cmake_cmd.arg(format!("-DCMAKE_INSTALL_PREFIX={}", install_prefix.to_string_lossy()));
    ...
    if !target_binary.exists() {
        ...
        println!("Manually copied llama.cpp server: {}", target_binary.display());
...
fn copy_runtime_files(bin_dir: &Path, target_dir: &Path, target: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(bin_dir)? {
        ...
        if file_name.ends_with(".dylib") || file_name.ends_with(".so") || file_name.ends_with(".dll") {
        ...
        if (target.contains("darwin") || target.contains("apple")) && 
           (file_name.ends_with(".metallib") || file_name.ends_with(".metal") || file_name.ends_with(".h")) {
            let target_path = target_dir.join(&*file_name);
            fs::copy(&path, &target_path)?;
...
fn cleanup_installation(llamacpp_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(llamacpp_dir)? {
        ...
        if path.is_file() {
            ...
            else if file_name.starts_with("llama-") && file_name != "llama-server" && file_name != "llama-server.exe" {
                fs::remove_file(&path)?;
    ...
    for entry in fs::read_dir(&bin_dir)? {
        ...
        if path.is_file() {
            ...
            fs::remove_file(&path)?;
            println!("Removed non-llama-server executable from bin/: {}", file_name);
        }
    }
...
fn ensure_runtime_libraries(build_dir: &Path, bin_dir: &Path, target: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn verify_binary_dependencies(binary_path: &Path, bin_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
...
fn find_binary(dir: &Path, names: &[&str]) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    Err(format!("Could not find binary in {}: tried {:?}", dir.display(), names).into())
}