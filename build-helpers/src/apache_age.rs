...
pub fn build(
    ...
    // Build Apache AGE extension in source directory
    build_age_extension(age_dir, &postgres_dir, target)?;

    // Copy built library file and control file to target directory
    let source_library_path = age_dir.join(library_name);
    if source_library_path.exists() {
        fs::copy(&source_library_path, &target_library_path)?;
    } else {
        return Err(format!("Apache AGE library not found after build: {}", source_library_path.display()).into());
    }
...
fn build_age_extension(
    ...
    if target.contains("darwin") {
        ...
        // Windows-specific makefile (if exists)
        let makefile_win = age_dir.join("Makefile.win");
        if makefile_win.exists() {
...
"#, sdk_path, original_pg_config, sdk_path, original_pg_config, sdk_path);
                ...
                let makefile_global = age_dir.join("postgresql-17.5.0/lib/pgxs/src/Makefile.global");
    ...
    if !output.status.success() {
        eprintln!("Error: Apache AGE extension build failed with exit code: {:?}", output.status.code());
