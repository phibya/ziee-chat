...
async fn main() -> Result<(), APIError> {
    ...
    let (pipelines, global_rank, daemon_manager) = if args.multi_process {
        ...
        let log_file = format!("candle_server-vllm-rank-{}.log", global_rank);
        ...
        let log_file = format!("candle_server-vllm-{}ranks.log", device_ids.len());
        let _ = config_log(logger, args.log, log_file);
        (
            loader
                .load_model(
    ...
    let (pipelines, global_rank) = {
        let log_file = "candle_server-vllm.log".to_string();
        let _ = config_log(logger, args.log, log_file);
        (
            loader
