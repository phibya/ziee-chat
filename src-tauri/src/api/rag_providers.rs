...
use crate::database::{
    models::{
        CreateRAGDatabaseRequest, CreateRAGProviderRequest, RAGDatabase, RAGProvider, RAGProviderListResponse,
        UpdateRAGDatabaseRequest, UpdateRAGProviderRequest,
    },
...
pub async fn clone_rag_provider(
    ...
    let provider = rag_providers::clone_rag_provider(provider_id)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn list_rag_provider_databases(
    ...
    let databases = rag_providers::list_rag_databases_by_provider(provider_id)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn add_database_to_rag_provider(
    ...
    let database = rag_providers::create_rag_database(provider_id, request)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn update_rag_database(
    ...
    let database = rag_providers::update_rag_database(database_id, request)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn delete_rag_database(
    ...
    rag_providers::delete_rag_database(database_id)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn start_rag_database(
    ...
    rag_providers::set_rag_database_active(database_id, true)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn stop_rag_database(
    ...
    rag_providers::set_rag_database_active(database_id, false)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn enable_rag_database(
    ...
    rag_providers::set_rag_database_enabled(database_id, true)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn disable_rag_database(
    ...
    rag_providers::set_rag_database_enabled(database_id, false)
        .await
        .map_err(|_e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
