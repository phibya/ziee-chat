...
use crate::database::{
    models::{
        CreateSystemRAGInstanceRequest, RAGInstance, RAGInstanceListResponse,
        UpdateRAGInstanceRequest,
    },
    queries::rag_instances::{
        create_system_rag_instance, delete_rag_instance, get_rag_instance,
...
pub async fn create_system_rag_instance_handler(
    ...
    let instance = create_system_rag_instance(request).await.map_err(|e| {
        (
...
pub async fn list_system_rag_instances_handler(
    ...
    let response = list_system_rag_instances(page, per_page)
        .await
        .map_err(|e| {
            (
                axum::http::StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn get_system_rag_instance_handler(
    ...
    match instance {
        ...
        Some(_) => Err((
            ...
            AppError::forbidden("Not a system instance"),
        )),
...
pub async fn update_system_rag_instance_handler(
    ...
    let existing = get_rag_instance(instance_id, auth_user.user.id)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, AppError::from(e)))?;

    match existing {
        Some(instance) if instance.is_system => {
            let updated_instance = update_rag_instance(instance_id, request)
                .await
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, AppError::from(e)))?;
...
pub async fn delete_system_rag_instance_handler(
    ...
    match existing {
        Some(instance) if instance.is_system => {
            let success = delete_rag_instance(instance_id)
                .await
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, AppError::from(e)))?;
