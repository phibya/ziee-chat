...
use crate::{
    api::{middleware::AuthenticatedUser, errors::{ApiResult2, AppError}},
    database::{
        models::{
...
pub async fn list_projects(
    ...
    let pool = get_database_pool().map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Database connection error")))?;
    ...
    match projects::list_projects(&pool, user.user_id, page, per_page, params.search).await {
        ...
        Err(e) => {
            ...
            Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to list projects")))
        }
    }
}
...
pub async fn create_project(
    ...
    if request.name.trim().is_empty() {
        return Err((StatusCode::BAD_REQUEST, AppError::new(crate::api::errors::ErrorCode::ValidMissingRequiredField, "Project name cannot be empty")));
    }
    ...
    match projects::create_project(&pool, user.user_id, &request).await {
        ...
        Err(e) => {
            ...
            Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to create project")))
        }
    }
}
...
pub async fn get_project(
    ...
    let project = match projects::get_project_by_id(&pool, project_id, user.user_id).await {
        ...
        Err(e) => {
            ...
            return Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to get project")));
        }
    };
    ...
    let conversations =
        match projects::list_project_conversations(&pool, project_id, user.user_id).await {
            Ok(Some(convs)) => convs,
            Ok(None) => return Err((StatusCode::NOT_FOUND, AppError::not_found("Project conversations"))),
            Err(e) => {
                eprintln!("Failed to get project conversations: {:?}", e);
                return Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to get project conversations")));
            }
        };
...
pub async fn update_project(
    ...
    match projects::update_project(&pool, project_id, user.user_id, &request).await {
        ...
        Err(e) => {
            ...
            Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to update project")))
        }
    }
}
...
pub async fn delete_project(
    ...
    match projects::delete_project(&pool, project_id, user.user_id).await {
        ...
        Err(e) => {
            ...
            Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to delete project")))
        }
    }
}
...
pub async fn link_conversation(
        ...
        .await
    {
        ...
        Ok(None) => Err((StatusCode::NOT_FOUND, AppError::not_found("Project or conversation"))),
        Err(e) => {
            eprintln!("Failed to link conversation to project: {:?}", e);
            Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to link conversation to project")))
        }
    }
}
...
pub async fn unlink_conversation(
    ...
    {
        Ok(true) => Ok((StatusCode::NO_CONTENT, StatusCode::NO_CONTENT)),
        Ok(false) => Err((StatusCode::NOT_FOUND, AppError::not_found("Project or conversation"))),
        Err(e) => {
            eprintln!("Failed to unlink conversation from project: {:?}", e);
            Err((StatusCode::INTERNAL_SERVER_ERROR, AppError::internal_error("Failed to unlink conversation from project")))
        }
    }
}
