...
) -> Result<ApiProxyServerConfig, Box<dyn std::error::Error + Send + Sync>> {
...
) -> Result<ApiProxyServerStatus, Box<dyn std::error::Error + Send + Sync>> {
    let instance = get_proxy_server_instance().await;
...
pub async fn try_autostart_proxy_server() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Err(e) = validate_proxy_config(&config).await {
        tracing::warn!(
            "API Proxy Server autostart skipped due to invalid config: {}",
            e
        );
    ...
    match start_proxy_server().await {
        Ok(()) => {
            tracing::info!(
                "API Proxy Server auto-started successfully on {}:{}{}",
                config.address,
                config.port,
                config.prefix
            );
...
async fn validate_proxy_config(
    config: &ApiProxyServerConfig,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let enabled_models =
        crate::database::queries::api_proxy_server_models::get_enabled_proxy_models().await?;
