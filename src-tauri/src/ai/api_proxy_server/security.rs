...
use super::{log_security_event, ProxyError};
...
impl SecurityValidator {
    ...
    pub async fn reload_trusted_hosts(
        &self,
    ...
    fn matches_cidr(&self, cidr: &str, client_ip: &str) -> Result<bool, ProxyError> {
        // Parse client IP
        let client_addr: IpAddr = client_ip
            .parse()
...
async fn load_trusted_hosts(
) -> Result<Vec<ApiProxyServerTrustedHost>, Box<dyn std::error::Error + Send + Sync>> {
