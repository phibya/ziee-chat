...
pub async fn load_text_content(file_id: Uuid) -> Result<Option<String>, Box<dyn std::error::Error + Send + Sync>> {
    FILE_STORAGE.read_text_content(file_id).await
}

/// Load image data for a file reference as base64
pub async fn load_image_as_base64(file_id: Uuid, filename: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn load_document_images_as_base64(file_id: Uuid) -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn get_file_content_for_local_provider(
    ...
    if file_ref.is_pdf() || file_ref.is_document() {
        let text_content = load_text_content(file_ref.file_id).await?.unwrap_or_default();
