...
impl ModelManager {
    ...
    fn calculate_model_size(
        ...
        if model_files.len() > 1 || total_size > 1_000_000_000 {
            // > 1GB
            info!("Model file breakdown:");
            for (path, size) in &model_files {
                let size_gb = *size as f64 / (1024.0 * 1024.0 * 1024.0);
                info!("  - {} ({:.2} GB)", path.display(), size_gb);
            }
        }
                ...
                if let Some(file_name) = path.file_name() {
                    ...
                    if self.is_model_file(&path, &file_name_str) {
                        ...
                        *total_size += file_size;
                        ...
                        if size_mb > 100.0 {
                            // Log files > 100MB
                            info!("Found model file: {} ({:.1} MB)", path.display(), size_mb);
                        }
                    }
                }
            } else if path.is_dir() {
    ...
    /// Get the path to the candle_server-server binary
    fn get_model_server_binary_path(
        ...
        // Look for candle_server-server binary in the same directory
        let model_server_path = current_dir.join("candle_server-server");
        ...
        if model_server_path.exists() {
            ...
            #[cfg(windows)]
            {
                let model_server_exe = current_dir.join("candle_server-server.exe");
            ...
            let target_debug = current_dir.join("../target/debug/candle_server-server");
            if target_debug.exists() {
                return Ok(target_debug);
            }

            let target_release = current_dir.join("../target/release/candle_server-server");
            if target_release.exists() {
                return Ok(target_release);
            }

            Err("candle_server-server binary not found".into())
        }
    }
            ...
            match client.get(&health_url).send().await {
                Ok(response) => {
                    if response.status().is_success() {
                        info!("Model server is healthy, checking ready status...");
                        ...
                        match client.get(&ready_url).send().await {
                            ...
                            Err(e) => {
                                info!("Ready endpoint not accessible yet: {}", e);
                            }
                        }
                    } else {
                        info!(
    ...
    fn is_model_server_process(&self, pid: u32) -> bool {
        #[cfg(unix)]
        {
                ...
                .output()
            {
                Ok(output) => {
                    let output_str = String::from_utf8_lossy(&output.stdout);
                    // Check if the process name contains "candle_server-server"
                    output_str.contains("candle_server-server")
                }
                Err(_) => {
    ...
    async fn write_lock_file(
        ...
        info!("Created lock file at: {}", lock_file_path.display());
        Ok(())
    }
    ...
    async fn remove_lock_file(
        ...
        if lock_file_path.exists() {
            ...
            info!("Removed lock file at: {}", lock_file_path.display());
        }
        Ok(())
    }
    ...
    pub async fn start_model(
        ...
        model_path: String,
        model_type: String,
        device_type: crate::ai::candle_server::DeviceType,
        ...
        // Get the candle_server-server binary path
        ...
        if matches!(device_type, crate::ai::candle_server::DeviceType::Cpu) {
            command.arg("--cpu");
        }
        ...
        info!("Starting candle_server-server process: {:?}", command);
        ...
        info!(
            "Model-server process spawned with PID: {}, port: {}",
            pid, port
        );
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                error!(
                    "Model-server process exited immediately with status: {}",
                    status
                );
                let _ = self.remove_lock_file(&model_path).await;
                return Err(format!("Model-server process failed to start: {}", status).into());
            }
            Ok(None) => {
                // Process is still running, detach it
                info!("Model-server process is running, waiting for health check...");
                // We don't want to wait for the child process, so we detach it
                // The process will continue running independently
                std::mem::forget(child);
            }
            Err(e) => {
                error!("Failed to check candle_server-server process status: {}", e);
                let _ = self.remove_lock_file(&model_path).await;
        ...
        if let Err(e) = self.wait_for_model_health(port, timeout_seconds).await {
            error!("Model server health check failed: {}", e);
        ...
        info!("Model server is healthy and ready on port {}", port);
    ...
    pub async fn stop_model(
        ...
        if self.is_process_running(lock_info.pid) {
            info!(
                "Terminating candle_server-server process with PID: {}",
                lock_info.pid
            );
            ...
            #[cfg(unix)]
            {
                ...
                if let Err(e) = signal::kill(pid, Signal::SIGTERM) {
                    warn!("Failed to send SIGTERM to process {}: {}", lock_info.pid, e);
                }
                ...
                if self.is_process_running(lock_info.pid) {
                    ...
                    if let Err(e) = signal::kill(pid, Signal::SIGKILL) {
                        error!("Failed to send SIGKILL to process {}: {}", lock_info.pid, e);
                    }
                }
            }
                ...
                match output {
                    Ok(result) => {
                        if !result.status.success() {
                            ...
                            warn!("Failed to terminate process {}: {}", lock_info.pid, stderr);
                        }
                    }
                    Err(e) => {
                        error!(
            ...
            if self.is_process_running(lock_info.pid) {
                ...
                info!("Process {} terminated successfully", lock_info.pid);
            }
        }
