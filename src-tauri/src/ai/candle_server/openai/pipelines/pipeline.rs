...
use crate::ai::candle_server::backend::progress::{progress_worker, ProgressReporter};
use crate::ai::candle_server::openai::logits_processor::LogitsProcessor;
use crate::ai::candle_server::openai::models::TokenID;
use crate::ai::candle_server::openai::requests::StopTokens;
use crate::ai::candle_server::openai::sampling_params::{Logprobs, TopLogprob};
use crate::ai::candle_server::openai::{BosEosToken, TokenizerConfig};
use crate::ai::candle_server::scheduler::sequence::SequenceGroup;
use crate::ai::candle_server::{
  openai::{
    conversation::{
...
impl DefaultLoader {
  ...
  pub async fn load_model(
    ...
    #[cfg(
      feature = "nccl"
    )] comm_id: Option<crate::openai::distributed::Id>, //must pass comm id in multiprocess mode
    ...
    #[cfg(
      feature = "nccl"
    )] global_rank: Option<usize>, //must pass current global rank in multi-node mode
    #[cfg(
      feature = "nccl"
    )] global_world_size: Option<usize>, //must pass total number of devices used in multi-node mode
    ...
    {
      let device = crate::ai::candle_server::new_device(device_ids[0]).unwrap();
      ...
      use crate::ai::candle_server::openai::distributed::Comm;
      #[cfg(feature = "nccl")]
      ...
      let results: Vec<_> = device_ids
        ...
        .map(|(rank, dev_id)| {
          ...
          let device = crate::ai::candle_server::new_device(*dev_id).unwrap();
          #[cfg(feature = "nccl")]
    ...
    let pipelines = models
      ...
      .map(|(rank, model)| {
        ...
        let (tokenizer, chat_template, bos_token, eos_token): (
          ...
          use crate::ai::candle_server::backend::gguf::{get_gguf_info, Content, GGUFInfo};
