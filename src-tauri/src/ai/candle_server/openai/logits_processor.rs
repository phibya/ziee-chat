#[cfg(feature = "cuda")]
use crate::ai::candle_server::backend::custom_ops::sort::ArgSortOp; //Use our custom sort kernel, fix kernel crash on A100
use candle_core::D;
use candle_core::{DType, Error, Result, Tensor};
use crate::ai::candle_server::openai::sampling_params::SamplingParams;
...
impl LogitsProcessor {
  ...
  fn sample_multinomial(&self, prs: &Vec<f32>) -> Result<u32> {
    let distr = rand::distributions::weighted::WeightedIndex::new(prs).map_err(Error::wrap)?;
