...
struct CandleStreamResponse {
    choices: Vec<CandleStreamChoice>,
}

#[derive(Debug, Deserialize)]
struct CandleStreamChoice {
    delta: CandleStreamDelta,
    finish_reason: Option<String>,
}

#[derive(Debug, Deserialize)]
struct CandleStreamDelta {
...
impl AIProvider for CandleProvider {
    async fn chat(
        ...
        let api_response: CandleResponse = response.json().await?;
                ...
                Ok(bytes) => {
                    ...
                    while let Some(line_end) = buffer_guard.find('\n') {
                        ...
                        if let Some(data) = line.strip_prefix("data: ") {
                            match serde_json::from_str::<CandleStreamResponse>(data) {
                                Ok(stream_response) => {
