...
pub fn create_provider(
    ...
    match config.provider_type.as_str() {
        "local" => Ok(Box::new(local::LocalAuthProvider::new(config)?)),
        "ldap" => Ok(Box::new(ldap::LdapAuthProvider::new(config)?)),
        "oauth2" | "oidc" => Ok(Box::new(oauth2::OAuth2Provider::new(config)?)),
        "saml" => Ok(Box::new(saml::SamlProvider::new(config)?)),
