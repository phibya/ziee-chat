...
impl PandocUtils {
    ...
    pub fn get_pandoc_path() -> Option<PathBuf> {
        ...
        if let Some(pandoc_path) = ResourcePaths::find_executable_binary("pandoc") {
            ...
            let test_result = Command::new(&pandoc_path)
                .arg("--version")
                .output();
        ...
        let system_test = Command::new("pandoc")
            .arg("--version")
            .output();
...
mod tests {
    ...
    fn test_pandoc_path_detection() {
        ...
        assert_eq!(PandocUtils::is_pandoc_available(), PandocUtils::get_pandoc_path().is_some());
    }
