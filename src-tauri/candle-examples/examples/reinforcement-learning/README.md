...
```bash
cargo run --example reinforcement-learning --features=pyo3 --package candle_server-examples -- pg
```

For the Deep Deterministic Policy Gradient example:

```bash
cargo run --example reinforcement-learning --features=pyo3 --package candle_server-examples -- ddpg
```
