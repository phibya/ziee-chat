...
impl CausalSelfAttention {
  fn apply_rotary_emb(&self, x: &Tensor, input_positions: &[Vec<usize>]) -> Result<Tensor> {
    ...
    for (b, seqlen_offset) in zip(0..b_sz, input_positions) {
      // Safe narrow with bounds checking
      let cos_dim = self.cos_sin_cache.cos.dim(0)?;
      let sin_dim = self.cos_sin_cache.sin.dim(0)?;
      let start_pos = seqlen_offset[0];
      
      // Clamp the sequence length to prevent out of bounds access
      let safe_seq_len = if start_pos + seq_len > cos_dim {
        if start_pos >= cos_dim {
          return Err(candle_core::Error::DimOutOfRange { 
            shape: candle_core::Shape::from_dims(&[cos_dim]), 
            dim: 0, 
            op: "narrow" 
          }.into());
        }
      ...
      let cos = self
        .cos_sin_cache
        .cos
        .narrow(0, start_pos, safe_seq_len)?;
      let sin = self
        .cos_sin_cache
        .sin
        .narrow(0, start_pos, safe_seq_len)?;
