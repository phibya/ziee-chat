    ...
    if (isSchemaReference(schema)) {
        ...
        if (typeName.startsWith('JsonOption_for_')) {
            ...
            if (actualType === 'Array_of_File') {
                ...
                var itemType = actualType.replace('Array_of_', '');
    ...
    if (schema.type === 'object' && schema.properties) {
        ...
        for (var _i = 0, _b = Object.entries(schema.properties); _i < _b.length; _i++) {
            ...
            if (isSchemaReference(propSchema)) {
                var refTypeName = extractSchemaName(propSchema.$ref);
...
function generateSSEEventType(name, oneOfVariants) {
    var eventTypes = [];
    for (var _i = 0, oneOfVariants_1 = oneOfVariants; _i < oneOfVariants_1.length; _i++) {
        var variant = oneOfVariants_1[_i];
        if (variant.type === 'object' && variant.properties) {
            // Extract the event name and data type from the variant
            var eventNames = Object.keys(variant.properties);
            if (eventNames.length === 1) {
                var eventName = eventNames[0];
                var eventDataSchema = variant.properties[eventName];
                var dataType = 'any';
                ...
                eventTypes.push("  ".concat(eventName, ": ").concat(dataType));
            }
        }
    }
    return "export type ".concat(name, " = {\n").concat(eventTypes.join('\n'), "\n}");
}
