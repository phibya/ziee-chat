...
import { Card, List, Button, Modal, Form, Input, Typography, Space, Avatar } from 'antd'
...
export function AssistantsPage() {
  const { assistants, createAssistant, updateAssistant, deleteAssistant } = useAppStore()
  ...
  return (
    <div style={{ padding: '24px', height: '100%', overflow: 'auto' }}>
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={handleCreateAssistant}>
          Create Assistant
        </Button>
      </div>
      ...
      <List
        ...
        renderItem={(assistant) => (
          <List.Item>
            <Card
              actions={[
                <EditOutlined key="edit" onClick={() => handleEditAssistant(assistant)} />,
                <DeleteOutlined key="delete" onClick={() => handleDeleteAssistant(assistant.id)} />,
              ]}
              hoverable
            >
              <Card.Meta
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: '16px' }}>
          <Form.Item
            label="Name"
            name="name"
            rules={[{ required: true, message: 'Please input the assistant name!' }]}
          >
          ...
          <Form.Item
            label="Description"
            name="description"
            rules={[{ required: true, message: 'Please input the assistant description!' }]}
          >
