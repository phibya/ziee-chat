...
export const getSystemServer = async (serverId: string): Promise<MCPServer> => {
  try {
    const server = await ApiClient.AdminMcp.getSystemServer({
      id: serverId,
    })

    // Update server in store if it exists
    useAdminMCPServersStore.setState(draft => {
      const index = draft.systemServers.findIndex(s => s.id === serverId)
      if (index >= 0) {
        draft.systemServers[index] = server
      }
    })

    return server
  } catch (error) {
    console.error('Failed to get system server:', error)
    throw error
  }
}
...
export const updateSystemServer = async (
  ...
  try {
    ...
    const updatedServer = await ApiClient.AdminMcp.updateSystemServer({
      id: serverId,
      ...data,
    })
...
export const deleteSystemServer = async (serverId: string): Promise<void> => {
  try {
    useAdminMCPServersStore.setState(draft => {
      draft.operationsLoading.set(serverId, true)
      draft.systemServersError = null
    ...
    await ApiClient.AdminMcp.deleteSystemServer({ id: serverId })

    useAdminMCPServersStore.setState(draft => {
      draft.systemServers = draft.systemServers.filter(
        server => server.id !== serverId,
      )
      draft.operationsLoading.delete(serverId)
    })
...
// System server lifecycle management
export const startSystemServer = async (
  serverId: string,
  ...
  try {
    ...
    const response = await ApiClient.AdminMcp.startSystemServer({
      id: serverId,
    })
    ...
    console.error('Failed to start system server:', error)
    useAdminMCPServersStore.setState(draft => {
      draft.operationsLoading.delete(`${serverId}-start`)
      draft.systemServersError =
        error instanceof Error ? error.message : 'Failed to start system server'
    })
    throw error
  }
}

export const stopSystemServer = async (
  serverId: string,
): Promise<ServerActionResponse> => {
  try {
    useAdminMCPServersStore.setState(draft => {
    ...
    const response = await ApiClient.AdminMcp.stopSystemServer({
      id: serverId,
    })
    ...
    console.error('Failed to stop system server:', error)
    useAdminMCPServersStore.setState(draft => {
      draft.operationsLoading.delete(`${serverId}-stop`)
      draft.systemServersError =
        error instanceof Error ? error.message : 'Failed to stop system server'
    })
    throw error
  }
}

export const restartSystemServer = async (
  serverId: string,
): Promise<ServerActionResponse> => {
  try {
    useAdminMCPServersStore.setState(draft => {
    ...
    const response = await ApiClient.AdminMcp.restartSystemServer({
      id: serverId,
    })
    ...
    console.error('Failed to restart system server:', error)
    useAdminMCPServersStore.setState(draft => {
      draft.operationsLoading.delete(`${serverId}-restart`)
      draft.systemServersError =
        error instanceof Error
          ? error.message
          : 'Failed to restart system server'
    })
    throw error
  }
}
