...
if (isTauriView) {
    ...
    const observer = new MutationObserver(() => {
        const decorumDiv = document.querySelector('div[data-tauri-decorum-tb]') as HTMLDivElement | null
        if (decorumDiv) {
            if (isMacOS) {
                decorumDiv.remove()
            } else {
                ...
                const dragRegion = decorumDiv.querySelector('div[data-tauri-drag-region]')
                if (dragRegion) {
                    dragRegion.remove()
                }
            }
